    ...
    for index in range(len(triangles)):
        ...
        if filter_triangles(z_min, normal, CameraRay, xxs, yys):
            ...
            lightRay = lightRay / np.sqrt(
                lightRay[0] * lightRay[0] + lightRay[1] * lightRay[1] + lightRay[2] * lightRay[2])
            ...
            denominator = 1 / ((proj_points[1][1] - proj_points[2][1]) * (proj_points[0][0] - proj_points[2][0]) +
                               (proj_points[2][0] - proj_points[1][0]) * (
                                           proj_points[0][1] - proj_points[2][1]) + 1e-32)
            ...
            for y in range(max(0, int(start[1])), min(SCREEN_H, int(stop[1] + 1))):
                ...
                for x in range(int(minx), int(maxx)):
                    ...
                    if z < z_buffer[x, y] and z >= min_z:
                        ...
                        if textured:
                            u = int(
                                (w0 * uv_points[0][0] + w1 * uv_points[1][0] + w2 * uv_points[2][0]) * z * text_size[0])
                            v = int(
                                (w0 * uv_points[0][1] + w1 * uv_points[1][1] + w2 * uv_points[2][1]) * z * text_size[1])
